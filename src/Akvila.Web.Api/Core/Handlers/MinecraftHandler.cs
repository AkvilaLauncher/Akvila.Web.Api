...
using Akvila.Web.Api.Dto.Minecraft.AuthLib;
...
{
    public static async Task<IResult> GetMetaData(HttpContext context, ISystemService systemService, IAkvilaManager akvilaManager,
        IOptions<ServerSettings> options)
    {
        var skinsAddresses = options.Value.SkinDomains.ToList();
        skinsAddresses.AddRange(await GetEnvironmentAddress(akvilaManager));
    ...
    private static async Task<string[]> GetEnvironmentAddress(IAkvilaManager akvilaManager)
    {
        var domains = new List<string>();
        var skinService = await akvilaManager.Integrations.GetSkinServiceAsync();
        var cloakService = await akvilaManager.Integrations.GetCloakServiceAsync();
    ...
    public static async Task<IResult> HasJoined(HttpContext context, IAkvilaManager akvilaManager, ISystemService systemService, string userName,
        string serverId,
        string? ip)
    {
        var user = await akvilaManager.Users.GetUserByName(userName);

        if (user is null || string.IsNullOrEmpty(userName) || user.IsBanned || await akvilaManager.Users.CanJoinToServer(user, serverId) == false)
        ...
        var textureProtocol = akvilaManager.LauncherInfo.StorageSettings.TextureProtocol.GetDisplayName()?.ToLower()
    ...
    public static async Task<IResult> Join(IAkvilaManager akvilaManager, JoinRequest joinDto)
    {
        bool validateUser = await akvilaManager.Users.ValidateUser(joinDto.SelectedProfile, joinDto.ServerId, joinDto.AccessToken);
    ...
    public static async Task<IResult> GetProfile(HttpContext context, IAkvilaManager akvilaManager, ISystemService systemService, string uuid,
        bool unsigned = false)
    {
        ...
        var user = await akvilaManager.Users.GetUserByUuid(guidUuid);
        ...
        var textureProtocol = akvilaManager.LauncherInfo.StorageSettings.TextureProtocol.GetDisplayName()?.ToLower() ?? TextureProtocol.Https.GetDisplayName().ToLower();
